...
class RemoteDataSourceImplTest {
    ...
    private val remoteDataSource = RemoteDataSourceImpl(newsListApi, newsSorter)
    ...
    fun `getTopHeadlines returns sorted articles`() = runTest {
        ...
        val response = BbcNewsResponse(
            ...
            status = "ok",
            totalResults = unsortedArticles.size
        )
        coEvery { newsListApi.getTopHeadlines(any()) } returns response

        val result = remoteDataSource.getTopHeadlines("bbc-news")

        val expectedSortedArticles = newsSorter.sort(unsortedArticles)
        assertThat(expectedSortedArticles).isEqualTo(result.articles)
    }
}
